...
export default function MediaPorRedeChart({ data }) {
  ...
  const chartData = useMemo(() => {
    ...
    filteredData.forEach(registro => {
      if (registro.rede) {
        if (!redeData[registro.rede]) {
          redeData[registro.rede] = [];
        }
        redeData[registro.rede].push(registro.qtd_caixas);
      }
    });

    // Calcular médias
    return Object.entries(redeData)
      .map(([rede, caixasArray]) => ({
        name: rede,
        media: Number((caixasArray.reduce((sum, val) => sum + val, 0) / caixasArray.length).toFixed(2)),
        total: caixasArray.reduce((sum, val) => sum + val, 0),
        registros: caixasArray.length
      }))
            ...
            <div className="flex flex-wrap gap-2">
              {periodButtons.map(({ days, label }) => (
                <Button
                  ...
                  className={selectedPeriod === days ? "bg-orange-600 hover:bg-orange-700" : ""}
                >
                  {label}
                </Button>
              ))}
        ...
        <div style={{ width: '100%', height: 400 }}>
          <ResponsiveContainer>
            <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
              ...
              />
              <YAxis 
                label={{ value: 'Média de Caixas', angle: -90, position: 'insideLeft' }}
              />
              <Tooltip 
                formatter={(value, name) => [`${value} caixas/registro`, 'Média']}
