...
export default function TopRankings({ resumos }) {
  ...
  const topClientesCaixas = Object.entries(clientes).sort(([,a], [,b]) => b.caixas - a.caixas).slice(0, 10)
  const topPromotoresTotal = Object.entries(promotores).sort(([,a], [,b]) => b.total - a.total).slice(0, 10)
  const topFretistasTotal = Object.entries(fretistas).sort(([,a], [,b]) => b.total - a.total).slice(0, 10)
            ...
            <div className="space-y-4">
              {topClientesCaixas.map(([nome, dados], index) => (
                <div key={nome} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                  ...
                  <Badge className={getBadgeColor(index)}>{dados.caixas} caixas</Badge>
                </div>
              ))}
            </div>
          ) : (
            ...
            <div className="space-y-4">
              {topPromotoresTotal.map(([nome, dados], index) => (
                <div key={nome} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                  ...
                  <Badge className={getBadgeColor(index)}>R$ {dados.total.toFixed(2)}</Badge>
                </div>
              ))}
            </div>
          ) : (
