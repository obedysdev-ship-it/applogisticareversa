...
export default function RedeColumnChart({ data }) {
  const chartData = useMemo(() => {
    ...
    return Object.entries(redeCount).map(([rede, caixas]) => ({ name: rede, caixas })).sort((a,b)=>b.caixas-a.caixas)
  }, [data])
      ...
      <CardContent style={{ height: 400 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
            ...
            <YAxis />
            <Tooltip formatter={(value) => [`${value} caixas`, 'Quantidade']} contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(8px)', border: '1px solid #e2e8f0', borderRadius: '0.75rem' }} />
            <Legend />
            <Bar dataKey="caixas" name="Quantidade de Caixas" fill="#16a34a" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
