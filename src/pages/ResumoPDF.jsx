...
export default function ResumoPDF() {
  ...
  useEffect(() => {
    ...
    if (data) {
      try {
        ...
        if (parsed.nomeArquivo) document.title = parsed.nomeArquivo
      } catch {}
    }
    setLoading(false)
  }, [])
  ...
  return (
    <div className="bg-gray-200">
      <style>{`
        @media print {
          body { margin:0; padding:0; background:white !important; -webkit-print-color-adjust: exact !important; color-adjust: exact !important; }
          @page { size: A4; margin: 1cm; }
          .screen-only { display: none !important; }
          .print-container { margin:0 !important; padding:0 !important; box-shadow:none !important; border:none !important; }
        }
      `}</style>
      ...
      <div className="print-container max-w-4xl mx-auto my-8 p-8 bg-white shadow-xl border border-gray-200">
        <div className="text-center mb-8">
          <img src="https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/58237d489_logisticareversa.png" alt="Logotipo GDM Logística Reversa" className="h-16 mx-auto mb-4" />
        ...
        <div className="bg-gray-50 p-6 rounded-lg mb-8" style={{ colorAdjust: 'exact', WebkitColorAdjust: 'exact' }}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-green-100 p-4 rounded-lg border-l-4 border-green-500" style={{ colorAdjust: 'exact', WebkitColorAdjust: 'exact' }}>
              <div className="flex items-center gap-2 mb-2"><span className="text-green-600">📊</span><span className="font-medium text-green-800">Total de Registros</span></div>
            ...
            <div className="bg-blue-100 p-4 rounded-lg border-l-4 border-blue-500" style={{ colorAdjust: 'exact', WebkitColorAdjust: 'exact' }}>
              <div className="flex items-center gap-2 mb-2"><span className="text-blue-600">📦</span><span className="font-medium text-blue-800">Total de Caixas</span></div>
            ...
            <div className="bg-emerald-100 p-4 rounded-lg border-l-4 border-emerald-500" style={{ colorAdjust: 'exact', WebkitColorAdjust: 'exact' }}>
              <div className="flex items-center gap-2 mb-2"><span className="text-emerald-600">💰</span><span className="font-medium text-emerald-800">Valor Total</span></div>
            ...
            <div className="bg-teal-100 p-4 rounded-lg border-l-4 border-teal-500" style={{ colorAdjust: 'exact', WebkitColorAdjust: 'exact' }}>
              <div className="flex items-center gap-2 mb-2"><span className="text-teal-600">💎</span><span className="font-medium text-teal-800">Total Geral</span></div>
              <p className="text-3xl font-bold text-teal-900">R$ {totalGeral.toFixed(2)}</p>
              <p className="text-xs text-teal-600">Fretista + Promotor</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8">
          <div className="print:break-before-page">
            <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><span className="text-blue-600">👥</span>Resumo por Cliente</h3>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead><tr className="border-b-2 border-gray-300"><th className="text-left py-2 px-3 font-semibold text-gray-700">Nome</th><th className="text-right py-2 px-3 font-semibold text-gray-700">Caixas</th><th className="text-right py-2 px-3 font-semibold text-gray-700">Valor (R$)</th></tr></thead>
                <tbody>
                  {resumoClientes.map((row, i) => (
                    <tr key={row.name} className={`border-b border-gray-200 ${i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`} style={{ colorAdjust: 'exact', WebkitColorAdjust: 'exact' }}>
                      <td className="py-2 px-3 text-gray-800">{row.name}</td>
                      <td className="py-2 px-3 text-right font-semibold text-blue-600">{row.caixas}</td>
                      <td className="py-2 px-3 text-right font-semibold text-green-600">{row.total.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="print:break-before-page">
            <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><span className="text-green-600">🎯</span>Resumo por Promotor</h3>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead><tr className="border-b-2 border-gray-300"><th className="text-left py-2 px-3 font-semibold text-gray-700">Nome</th><th className="text-right py-2 px-3 font-semibold text-gray-700">Caixas</th><th className="text-right py-2 px-3 font-semibold text-gray-700">Valor (R$)</th></tr></thead>
                <tbody>
                  {resumoPromotores.map((row, i) => (
                    <tr key={row.name} className={`border-b border-gray-200 ${i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`} style={{ colorAdjust: 'exact', WebkitColorAdjust: 'exact' }}>
                      <td className="py-2 px-3 text-gray-800">{row.name}</td>
                      <td className="py-2 px-3 text-right font-semibold text-blue-600">{row.caixas}</td>
                      <td className="py-2 px-3 text-right font-semibold text-green-600">{row.total.toFixed(2)}</td>
          ...
          <div className="print:break-before-page">
            <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><span className="text-purple-600">🚛</span>Resumo por Fretista</h3>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead><tr className="border-b-2 border-gray-300"><th className="text-left py-2 px-3 font-semibold text-gray-700">Nome</th><th className="text-right py-2 px-3 font-semibold text-gray-700">Caixas</th><th className="text-right py-2 px-3 font-semibold text-gray-700">Valor (R$)</th></tr></thead>
                <tbody>
                  {resumoFretistas.map((row, i) => (
                    <tr key={row.name} className={`border-b border-gray-200 ${i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`} style={{ colorAdjust: 'exact', WebkitColorAdjust: 'exact' }}>
                      <td className="py-2 px-3 text-gray-800">{row.name}</td>
                      <td className="py-2 px-3 text-right font-semibold text-blue-600">{row.caixas}</td>
                      <td className="py-2 px-3 text-right font-semibold text-green-600">{row.total.toFixed(2)}</td>
          ...
          <div className="print:break-before-page">
            <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><span className="text-indigo-600">🏢</span>Resumo por Rede</h3>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead><tr className="border-b-2 border-gray-300"><th className="text-left py-2 px-3 font-semibold text-gray-700">Nome</th><th className="text-right py-2 px-3 font-semibold text-gray-700">Caixas</th><th className="text-right py-2 px-3 font-semibold text-gray-700">Valor (R$)</th></tr></thead>
                <tbody>
                  {resumoRedes.map((row, i) => (
                    <tr key={row.name} className={`border-b border-gray-200 ${i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`} style={{ colorAdjust: 'exact', WebkitColorAdjust: 'exact' }}>
                      <td className="py-2 px-3 text-gray-800">{row.name}</td>
                      <td className="py-2 px-3 text-right font-semibold text-blue-600">{row.caixas}</td>
                      <td className="py-2 px-3 text-right font-semibold text-green-600">{row.total.toFixed(2)}</td>
          ...
          <div className="print:break-before-page">
            <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><span className="text-red-600">📍</span>Resumo por UF</h3>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead><tr className="border-b-2 border-gray-300"><th className="text-left py-2 px-3 font-semibold text-gray-700">Nome</th><th className="text-right py-2 px-3 font-semibold text-gray-700">Caixas</th><th className="text-right py-2 px-3 font-semibold text-gray-700">Valor (R$)</th></tr></thead>
                <tbody>
          ...
          <div className="print:break-before-page">
            <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><span className="text-amber-600">👨‍💼</span>Resumo por Vendedor</h3>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead><tr className="border-b-2 border-gray-300"><th className="text-left py-2 px-3 font-semibold text-gray-700">Nome</th><th className="text-right py-2 px-3 font-semibold text-gray-700">Caixas</th><th className="text-right py-2 px-3 font-semibold text-gray-700">Valor (R$)</th></tr></thead>
                <tbody>
                  {resumoVendedores.map((row, i) => (
                    <tr key={row.name} className={`border-b border-gray-200 ${i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`} style={{ colorAdjust: 'exact', WebkitColorAdjust: 'exact' }}>
                      <td className="py-2 px-3 text-gray-800">{row.name}</td>
                      <td className="py-2 px-3 text-right font-semibold text-blue-600">{row.caixas}</td>
                      <td className="py-2 px-3 text-right font-semibold text-green-600">{row.total.toFixed(2)}</td>
          ...
          <div className="print:break-before-page">
            <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><span className="text-indigo-600">📋</span>Registros Detalhados</h3>
            <div className="overflow-x-auto">
